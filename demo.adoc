...
:title:	设置文档标题（可替代 = 标题）
:author: / :email:	设置作者信息
:revnumber: / :revdate:	设置版本号和日期
:description:	设置 meta 描述信息
:keywords:	设置 meta 关键词
:lang:	设置语言（如 zh-CN）
:doctype:	设置文档类型（如 book, article, manpage）
:imagesdir:	设置图片目录路径
:homepage:	设置项目主页链接
:data-uri:	将图片嵌入为 base64
:numbered:	启用自动编号（等效于 :sectnums:）
:backend:	设置输出格式（如 html5, pdf, docbook）
:experimental:	启用实验性语法（如 kbd:[Ctrl+C]）
:toc-title:	自定义目录标题（默认是 “Table of Contents”）
----
...
link:https://docs.asciidoctor.org/asciidoc/latest/[所有的语法链接]
...
我们的 `css` 类里面定制了 `gt` 这个属性，我们用上看下效果。

A [.gt]#custom role# must be fulfilled by the theme.
...
xx<x>中文dedg</x>xx   <z>斜体</z>     <h>高亮</h>后面的内容不高亮                     |              |
              xx                                                                      |
...
| modeA  | --> | modeB  |  <l h="https://example.com">中文kxx</l>                     |              |               
+--------+     +--------+  <l h="#xggegemoduleB">    </l>                             |              |
xx<x>    dedg</x>xx   <z>    </z>     <h>    </h>                                     |              |
              xx                                                                      |
<l h="#xggegemoduleB">    </l>           other                                        |
</pre>
++++
...
[#section-图像]
image::pics/2025-08-26.png[系统架构图,align="center",width=300]

[#section-图像]
image::pics/2025-08-26.png[系统架构图,align="left",width=300]

[#section-图像]
image::pics/2025-08-26.png[系统架构图,align="right",width=300]
...
真的可以 [.rt]#这是红色文字# 这样的啊              [.rt]#这是红色文字#

    语法的前后必须有空格，不然颜色不会正常显示。
        个的e


这是pass:[<span class="rt">红色文字</span>]的一部分。

这样也可以+++<span class="rt">红色文字</span>+++可以不保留空格
...
跳转到第一章的某个点：<<chapter1-anchor>>。

<<chapter1-anchor, 点我跳到第一章的某个点>>
